...
#define MICRO_MODULE_ERROR_LOCATING_FUNC_SYMBOL  -12
#define _MICRO_MODULE_ERROR_MAX                  -13
...
typedef int(*micro_module_generic_fn)(void*);
...
// Call [func_name] form [module_name] with [arg]
//
// The function must have the signature defined in
// `micro_module_generic_fn`
MICRO_MODULE_DEF int
micro_module_call(MicroModule *mm, const char* module_name,
...
micro_module_call(MicroModule *mm, const char* module_name,
                  const char* func_name, void* arg)
{
  if (!mm || !module_name || !func_name)
    return MICRO_MODULE_ERROR_IS_NULL;
  ...
  while (it)
  {
    ...
    {
      micro_module_generic_fn func;
      *(void**)(&func) = dlsym(it->module.dlhandler, func_name);
      if (!func)
        return MICRO_MODULE_ERROR_LOCATING_FUNC_SYMBOL;
